...
import { PAYMENT_METHODS, PaymentProvider, verifyPaymentStatus } from "../lib/payments";

interface PaymentCheckoutProps {
  txRef: string;
  onVerificationComplete: (status: "PAID" | "FAILED", data: any) => void;
  onCancel: () => void;
}

export const PaymentCheckout: React.FC<PaymentCheckoutProps> = ({ txRef, onVerificationComplete, onCancel }) => {
  const { t, formatCurrency, formatLocalizedDate } = useLocalization();
  const [transaction, setTransaction] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [processingPayment, setProcessingPayment] = useState(false);
  const [paymentStep, setPaymentStep] = useState<"method" | "simulating_gateway" | "verifying_with_server">("method");
  ...
  if (errorMsg || !transaction) {
    return (
      <div className="max-w-md mx-auto p-8 text-center bg-white rounded-2xl border border-slate-100 shadow-sm">
        ...
        <p className="text-xs text-slate-500 mb-6">{errorMsg || "Transaction could not be synchronized."}</p>
        <button onClick={onCancel} className="px-5 py-2 text-xs font-bold bg-slate-900 text-white rounded-lg">
          {t("common.back")}
        </button>
      </div>
    );
      ...
      {paymentStep === "method" && (
        <div className="p-6 sm:p-8">
          <div className="flex justify-between items-start border-b border-slate-100 dark:border-slate-900 pb-5 mb-6">
            <div>
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Transaction Invoice</span>
              <h2 className="text-lg font-black text-slate-900 dark:text-white mt-0.5">{transaction.description}</h2>
            ...
            {PAYMENT_METHODS.map((method) => {
              ...
              return (
                ...
                >
                  ...
                  <div>
                    <span className="font-bold text-slate-900 dark:text-white block">{method.name}</span>
                    <span className="text-[11px] text-slate-400 mt-1 block leading-relaxed">{method.instructions}</span>
                  </div>
                </button>
              );
            })}
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 mb-6 border border-slate-100 dark:border-slate-800">
              ...
              {selectedProvider === "chapa" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Cardholder Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Card Number</label>
                    <input
                      type="text"
                      placeholder="•••• •••• •••• ••••"
                      value={cardNumber}
              ...
              {(selectedProvider === "telebirr" || selectedProvider === "cbe_birr") && (
                <div>
                  <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1.5">Registered Wallet Phone</label>
                  <input
                    type="text"
              ...
              {selectedProvider === "bank_transfer" && (
                <div className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                  Please initiate a bank transfer from your mobile banking app to CBE A/C: <strong className="text-slate-900 dark:text-white">1000123456789</strong>.
                  Upon receipt, our financial audit team matches the reference ID <span className="font-mono text-blue-600 font-bold">{transaction.txRef}</span>.
                </div>
              )}
            </div>
        ...
        <div className="p-8 text-center min-h-[350px] flex flex-col items-center justify-center">
          ...
          {paymentStep === "simulating_gateway" && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto leading-relaxed">
                We have securely redirected your authorization request. Completing 3-D secure challenge queries for <span className="font-bold text-blue-600">{selectedProvider.toUpperCase()}</span>.
              </p>
            </div>
          )}
          ...
          {paymentStep === "verifying_with_server" && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto leading-relaxed">
                Querying Syntax's server-side secure payment callback webhook. Validating transaction authenticity...
              </p>
            </div>
          )}
        </div>
