...
export const TrainingAcademy: React.FC<TrainingAcademyProps> = ({
  ...
  const [currentView, setCurrentView] = useState<"overview" | "detail" | "corporate" | "register">(
    "overview",
  );
  ...
  // Initial deep link / preselection handling — waits for the catalog to
  // load since courses no longer exist synchronously.
  useEffect(() => {
    if (initialCourseTitle && coursesState.status === "success") {
      const match = coursesState.data.find((c) =>
        c.title.toLowerCase().includes(initialCourseTitle.toLowerCase()),
      );
      if (match) {
  ...
  }, [initialCourseTitle, coursesState]);

  // Available categories extracted from the fetched catalog
  const categories = useMemo(() => {
    if (coursesState.status !== "success") return ["all"];
    const list = new Set<string>();
    coursesState.data.forEach((c) => {
  ...
  const filteredCourses = useMemo(() => {
    ...
    return coursesState.data.filter((course) => {
      const matchesSearch =
        ...
        course.skills_gained.some((s) => s.toLowerCase().includes(searchQuery.toLowerCase()));
      const matchesCategory =
        selectedCategory === "all" || course.category?.name === selectedCategory;
      const matchesMode = selectedMode === "all" || formatCourseMode(course.mode) === selectedMode;
      const matchesLevel =
        selectedLevel === "all" || formatCourseLevel(course.level) === selectedLevel;

      return matchesSearch && matchesCategory && matchesMode && matchesLevel;
    });
  }, [coursesState, searchQuery, selectedCategory, selectedMode, selectedLevel]);

  // Selected Course Object
  const selectedCourse = useMemo(() => {
    if (coursesState.status !== "success") return null;
    return coursesState.data.find((c) => c.id === selectedCourseId) || null;
  ...
  const handleViewCourseDetail = (courseId: number) => {
    ...
    const courseObj =
      coursesState.status === "success"
        ? coursesState.data.find((c) => c.id === courseId)
        : undefined;
    if (courseObj) {
  ...
  return (
    <div id="training-academy-root" className="space-y-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-6">
        <div>
          <span className="text-[10px] text-purple-600 dark:text-purple-400 font-bold uppercase tracking-wider block font-mono">
            Hands-on Academy Hub
          </span>
      ...
      {coursesState.status === "error" && (
        <div className="p-6 rounded-2xl bg-red-50/50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/30 text-center space-y-1 max-w-xl mx-auto">
          <p className="text-xs font-bold text-red-600 dark:text-red-400">
            Couldn't load our course catalog.
          </p>
          <p className="text-[11px] text-red-500/80 dark:text-red-400/70">
            {coursesState.error.message}
          </p>
        </div>
      )}
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="p-5 bg-purple-50/50 dark:bg-purple-950/10 border border-purple-100/50 dark:border-purple-900/30 rounded-2xl flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                      Configure actual optical fiber switches, CCTV networks, and Suprema biometric
                      locks.
                    </p>
                  </div>
                </div>
                ...
                <div className="p-5 bg-blue-50/50 dark:bg-blue-950/10 border border-blue-100/50 dark:border-blue-900/30 rounded-2xl flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-tight">
                      8Y Verified Instructors
                    </h4>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                      Courses designed and led by active deployment engineers with international
                      security badges.
                    </p>
                  </div>
                </div>
                ...
                <div className="p-5 bg-emerald-50/50 dark:bg-emerald-950/10 border border-emerald-100/50 dark:border-emerald-900/30 rounded-2xl flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                      Syntax Technology certificates are recognized across corporate security and
                      telecom contractors.
                    </p>
                  </div>
                </div>
                ...
                <div className="p-5 bg-amber-50/50 dark:bg-amber-950/10 border border-amber-100/50 dark:border-amber-900/30 rounded-2xl flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-tight">
                      Direct Career Routing
                    </h4>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">
                      High-performing graduates are directly routed to Syntax deployment
                      sub-contracting pools.
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-1.5 pt-1">
                        ...
                        <ul className="space-y-1 text-[11px] text-slate-600 dark:text-slate-300">
                          {course.skills_gained.slice(0, 3).map((skill, idx) => (
                ...
                <div className="text-center py-16 border border-dashed border-slate-200 dark:border-slate-850 rounded-2xl max-w-xl mx-auto space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-500">
                      {coursesState.data.length === 0
                        ? "Check back soon — new academy courses are added regularly."
              ...
              <div className="p-8 bg-slate-900 text-white rounded-3xl border border-slate-800 relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-8 max-w-5xl mx-auto">
                ...
                <div className="relative space-y-3 max-w-xl">
                  ...
                  <h3 className="text-xl sm:text-2xl font-black uppercase tracking-tight">
                    Need custom-designed group cohorts for your technical teams?
                  </h3>
                  <p className="text-xs text-slate-300 leading-relaxed">
                    We custom configure curricula specifically around your deployed workplace
                    assets. We deliver direct hands-on testing tools, structured laboratory kits,
                    and on-premises instruction for 5+ staff members.
                  </p>
                </div>
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono font-bold text-slate-400">
                  Course Reference: {selectedCourse.slug}
                </span>
              </div>
              ...
              <div className="p-6 sm:p-10 bg-slate-900 text-white rounded-3xl border border-slate-800 shadow-xl relative overflow-hidden space-y-6">
                ...
                <div className="relative max-w-4xl space-y-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-[9px] font-bold px-2.5 py-1 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30 font-mono uppercase tracking-wider">
                      {selectedCourse.category?.name || "General"}
                    </span>
                    <span className="text-[9px] font-bold px-2.5 py-1 rounded-full bg-slate-800 text-slate-300 border border-slate-700 font-mono uppercase tracking-wider">
                      {formatCourseLevel(selectedCourse.level)}
                    </span>
                    {getDeliveryBadge(formatCourseMode(selectedCourse.mode))}
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-slate-800 text-xs max-w-3xl">
                    ...
                    <div>
                      ...
                      <span className="font-bold text-emerald-400 flex items-center gap-1.5 mt-0.5">
                        ...
                        {formatCoursePrice(selectedCourse.price, "Contact for pricing")}
                      </span>
                    </div>
                    <div className="col-span-2 sm:col-span-2">
                      <span className="block text-[9px] text-slate-400 uppercase tracking-widest font-mono">
                        Upcoming Intake Schedule
                      </span>
                      <span className="font-bold text-slate-200 flex items-center gap-1.5 mt-0.5">
                        <Calendar className="w-4 h-4 text-amber-400 shrink-0" />{" "}
                        {selectedCourse.schedule || "Register to view custom schedules"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="relative pt-4 flex flex-wrap gap-3 border-t border-slate-800">
                  ...
                  {selectedCourse.mode === "corporate" || selectedCourse.price.amount === null ? (
                    <button
                      onClick={() => setCurrentView("corporate")}
                ...
                <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="bg-white dark:bg-slate-950 p-6 rounded-2xl border border-slate-100 dark:border-slate-850 space-y-3">
                    <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                      <Users className="w-4 h-4 text-purple-500" /> Target Audience (Who Is This
                      For?)
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                      {selectedCourse.target_audience?.map((aud, idx) => (
                        <div
                          key={idx}
                          className="flex items-center gap-2.5 p-3 bg-slate-50 dark:bg-slate-900 rounded-xl"
                        >
                  ...
                  <div className="bg-white dark:bg-slate-950 p-6 rounded-2xl border border-slate-100 dark:border-slate-850 space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                        <BookOpen className="w-4 h-4 text-purple-500" /> Course Content & Lab
                        Modules
                      </h3>
                      <span className="text-[10px] font-bold text-slate-400 bg-slate-50 dark:bg-slate-900 px-2 py-0.5 rounded-md uppercase">
                        Lab-centric
                      </span>
                    </div>
                    ...
                    {selectedCourse.modules ? (
                      ...
                      <div className="space-y-2">
                        {selectedCourse.syllabus.map((syl, idx) => (
                          <div
                            key={idx}
                            className="p-3 bg-slate-550/20 dark:bg-slate-900/40 rounded-xl border border-slate-100 dark:border-slate-850 flex items-start gap-3"
                          >
                            <span className="w-5 h-5 rounded bg-purple-100 dark:bg-purple-950 text-purple-600 dark:text-purple-400 flex items-center justify-center font-mono font-bold text-[10px] shrink-0">
                              {idx + 1}
                            </span>
                            <span className="text-xs text-slate-700 dark:text-slate-350 leading-relaxed mt-0.5">
                              {syl}
                            </span>
                          </div>
                        ))}
                ...
                <div className="space-y-8">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-snug mt-1">
                            {selectedCourse.location ||
                              "Online via remote virtual server desktop access"}
                          </p>
                        </div>
                      </div>
                  ...
                  <div className="bg-white dark:bg-slate-950 p-6 rounded-2xl border border-slate-100 dark:border-slate-850 space-y-3">
                    <h4 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                      <Sparkles className="w-3.5 h-3.5 text-purple-500" /> Career Resume Skills
                      Gained
                    </h4>
                    <div className="space-y-2">
                      {selectedCourse.skills_gained.map((skill, idx) => (
                        <div
                          key={idx}
                          className="flex items-center gap-2 text-xs font-semibold text-slate-700 dark:text-slate-350"
                        >
                  ...
                  <div className="p-6 bg-purple-600 text-white rounded-2xl text-center space-y-4">
                    ...
                    <div className="space-y-1">
                      <h4 className="text-sm font-black uppercase tracking-tight">
                        Reserve Seat Instantly
                      </h4>
                      <p className="text-[11px] text-purple-100">
                        Only 12 seats per lab cohort to guarantee dedicated instructor-to-student
                        workspace access.
                      </p>
                    </div>
                    <button
              ...
              <div className="bg-slate-50 dark:bg-slate-900 p-6 sm:p-10 rounded-3xl border border-slate-100 dark:border-slate-800 space-y-6 max-w-4xl mx-auto">
                <div className="text-center space-y-1.5">
                  <span className="text-[10px] text-purple-600 dark:text-purple-400 font-mono font-bold uppercase tracking-wider">
                    Quick Application Form
                  </span>
                  <h3 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">
                    Register Interest / Apply for {selectedCourse.title}
                  </h3>
                  <p className="text-xs text-slate-500 max-w-xl mx-auto">
                    Complete this simple intake form to register your details. An advisor will
                    contact you with fee invoices and lab schedules.
                  </p>
                </div>
                ...
                {regSuccess ? (
                  <div className="p-6 bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900 text-center rounded-2xl max-w-lg mx-auto space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        We have queued your training application into our student registry system.
                  ...
                  <form onSubmit={handleRegisterSubmit} className="space-y-4 max-w-3xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setRegFormData((p) => ({ ...p, organization: e.target.value }))
                          }
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setRegFormData((p) => ({ ...p, experience: e.target.value }))
                          }
                          className="w-full px-3 py-2 text-xs border border-slate-250 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                        >
                          ...
                          <option value="Intermediate (Some IT or security experience)">
                            Intermediate (Some IT or security experience)
                          </option>
                          <option value="Advanced (Working network technician/installer)">
                            Advanced (Working network technician/installer)
                          </option>
                        </select>
                      </div>
                    </div>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">
                        Specific Career or Business Goals
                      </label>
                      <textarea
            ...
            >
              ...
              <div className="p-8 sm:p-12 bg-slate-900 text-white rounded-3xl border border-slate-800 relative overflow-hidden space-y-6">
                ...
                <div className="relative max-w-4xl space-y-4">
                  ...
                  <p className="text-xs sm:text-sm text-slate-300 leading-relaxed max-w-3xl">
                    Synchronize your internal workforce with physical security setups, modern
                    networking configurations, and IT optimization policies. Syntax Technology
                    delivers practical, instructor-guided bootcamps custom aligned with your
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-6 bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-850 rounded-2xl space-y-3">
                  ...
                  <h4 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">
                    Active Lab Simulators
                  </h4>
                  <p className="text-xs text-slate-500 leading-relaxed">
                    We don't rely on slides. We ship real IP surveillance cameras, network terminal
                    routers, and biometric door locking assemblies straight to your premises for
                    live configuration labs.
                  </p>
                </div>
                ...
                <div className="p-6 bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-850 rounded-2xl space-y-3">
                  ...
                  <h4 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">
                    Adaptive Flexible Hours
                  </h4>
                  <p className="text-xs text-slate-500 leading-relaxed">
                    Minimize company operational interruptions. We orchestrate early morning
                    modules, weekend bootcamps, or sequential evening cohorts that synchronize with
                    standard employee shift schedules.
                  </p>
                </div>
                ...
                <div className="p-6 bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-850 rounded-2xl space-y-3">
                  ...
                  <h4 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">
                    Post-Academy Field SLA
                  </h4>
                  <p className="text-xs text-slate-500 leading-relaxed">
                    Every corporate program includes a 3-month ongoing support bridge. Instructors
                    provide direct diagnostic audits on-premise to ensure employees successfully
                    deploy their knowledge on active projects.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-900 p-6 sm:p-10 rounded-3xl border border-slate-100 dark:border-slate-800 space-y-6 max-w-4xl mx-auto">
                <div className="text-center space-y-1">
                  ...
                  <h3 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">
                    Request Corporate Group Training
                  </h3>
                  <p className="text-xs text-slate-500 max-w-xl mx-auto">
                    Complete this checklist specification. A corporate director from Syntax will
                    evaluate your organizational goals and issue a formal stamped PDF proposal
                    within 24 hours.
                  </p>
                </div>
                ...
                {corpSuccess ? (
                  <div className="p-6 bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900 text-center rounded-2xl max-w-lg mx-auto space-y-4">
                    ...
                    <div>
                      <h4 className="text-base font-bold text-slate-900 dark:text-white">
                        Corporate Query Logged!
                      </h4>
                      <p className="text-xs text-slate-500 mt-1">
                        Your corporate upskilling parameters have been routed to our corporate
                        accounts division. A program coordinator will call you shortly to define
                        curriculum specs.
                      </p>
                    </div>
                    <button
                  ...
                  <form onSubmit={handleCorporateSubmit} className="space-y-4 max-w-3xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setCorpFormData((p) => ({ ...p, company: e.target.value }))
                          }
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setCorpFormData((p) => ({ ...p, email: e.target.value }))
                          }
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setCorpFormData((p) => ({ ...p, phone: e.target.value }))
                          }
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">
                          Estimated Cohort Student Count
                        </label>
                        <select
                          value={corpFormData.cohortSize}
                          onChange={(e) =>
                            setCorpFormData((p) => ({ ...p, cohortSize: e.target.value }))
                          }
                          className="w-full px-3 py-2 text-xs border border-slate-250 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                        >
                          <option value="1-4 participants">1-4 participants</option>
                          <option value="5-10 participants">
                            5-10 participants (Recommended for dedicated hardware kits)
                          </option>
                          <option value="11-20 participants">11-20 participants</option>
                          <option value="20+ participants">
                            20+ participants (Multi-session bootcamps)
                          </option>
                        </select>
                      </div>
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setCorpFormData((p) => ({ ...p, locationPreference: e.target.value }))
                          }
                          className="w-full px-3 py-2 text-xs border border-slate-250 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                        >
                          <option value="On-site at our premises">
                            On-site at our corporate premises
                          </option>
                          <option value="At Syntax Training Center">
                            At Syntax Executive Training Labs
                          </option>
                          <option value="Fully Virtual Remote Labs">
                            Fully Virtual (Online with hardware simulators)
                          </option>
                        </select>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">
                          Select Main Course Template
                        </label>
                        <select
                          value={corpFormData.courseInterest}
                          onChange={(e) =>
                            setCorpFormData((p) => ({ ...p, courseInterest: e.target.value }))
                          }
                          className="w-full px-3 py-2 text-xs border border-slate-250 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                        >
                          ...
                          <option value="Custom multi-disciplinary security & IT specs">
                            Custom multi-disciplinary security & IT specs
                          </option>
                        </select>
                      </div>
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setCorpFormData((p) => ({ ...p, preferredTimeline: e.target.value }))
                          }
                          className="w-full px-3 py-2 text-xs border border-slate-250 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                        >
                          <option value="Immediate (Within 14 days)">
                            Immediate (Within 14 days)
                          </option>
                          <option value="Within 30 days">Within 30 days</option>
                          <option value="Next quarter (Planning ahead)">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">
                        Tailor-Made Requests & System Details
                      </label>
                      <textarea
                        value={corpFormData.customSpecs}
                        onChange={(e) =>
                          setCorpFormData((p) => ({ ...p, customSpecs: e.target.value }))
                        }
                        rows={4}
                    ...
                    >
                      {submittingCorp ? (
                        ...
                        <>
                          <Send className="w-4 h-4 text-emerald-450" /> Submit Corporate Cohort
                          Query
                        </>
                      )}
                    </button>
